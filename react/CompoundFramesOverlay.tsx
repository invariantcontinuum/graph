  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      for (const [src, box] of boxes) {
        const tlX = worldToScreenX(box.minX, box.minY, vp, cvs.width);
        const tlY = worldToScreenY(box.minX, box.minY, vp, cvs.height);
        const brX = worldToScreenX(box.maxX, box.maxY, vp, cvs.width);
        const brY = worldToScreenY(box.maxX, box.maxY, vp, cvs.height);
        const PAD = 24;
        const rx = Math.min(tlX, brX) - PAD;
        const ry = Math.min(tlY, brY) - PAD;
        const w = Math.abs(brX - tlX) + PAD * 2;
        const h = Math.abs(brY - tlY) + PAD * 2;
        const r = 12;
        ctx.beginPath();
        ctx.moveTo(rx + r, ry);
