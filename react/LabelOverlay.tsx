...
export function LabelOverlay({
  ...
  const labelCacheRef = useRef<
    Map<string, { raw: string; text: string; glyph: string | null; type: string; typeTag: string | null; fullText: string; chars: string[] }>
  >(new Map());
...
interface FrameContext {
  ...
  labelCache: Map<string, { raw: string; text: string; glyph: string | null; type: string; typeTag: string | null; fullText: string; chars: string[] }>;
...
function drawOneLabel(
  ...
  if (!cached || cached.raw !== rawLabel || cached.glyph !== glyph || cached.type !== type) {
    ...
    cached = { raw: rawLabel, text, glyph, type, typeTag, fullText, chars };
