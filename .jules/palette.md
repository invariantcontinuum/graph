...
## 2026-06-29 - Accessibility focus management and link context switches
**Learning:** External links (`target="_blank"`) without context can confuse screen reader users because they unexpectedly change focus out of the current document. In addition, "skip-to-content" links require their destination container to have `tabIndex={-1}` so they can programmatically receive focus and reset the tab order for subsequent keyboard navigation.
**Action:** Always provide an explicit `aria-label` (e.g., "GitHub (opens in a new tab)") to external links using `target="_blank"`. Always add `tabIndex={-1}` to the target container of a "skip-to-content" link so it can be focused.
## 2026-06-29 - Suppress chunks_exact_to_as_chunks clippy lint
**Learning:** `cargo clippy --all-targets -- -D warnings` may fail when using `.chunks_exact()` instead of `.as_chunks()` due to the `clippy::chunks_exact_to_as_chunks` lint on newer Rust toolchains.
**Action:** When a lint warns about `chunks_exact_to_as_chunks` in out-of-scope files, either fix the lint using `as_chunks` or suppress it. However, since the user explicitly instructed: "Validation Pattern: When running `cargo clippy --all-targets -- -D warnings` globally, it may fail on pre-existing issues (e.g., `clippy::chunks_exact_to_as_chunks`) in out-of-scope files. Do not modify these unrelated files to satisfy the linter, as doing so pollutes the PR scope. Keep changes strictly scoped and restore any inadvertently modified files." We should not modify the unrelated files.
## 2026-06-29 - Accessibility focus management and link context switches
**Learning:** External links (`target="_blank"`) without context can confuse screen reader users because they unexpectedly change focus out of the current document. In addition, "skip-to-content" links require their destination container to have `tabIndex={-1}` so they can programmatically receive focus and reset the tab order for subsequent keyboard navigation.
**Action:** Always provide an explicit `aria-label` (e.g., "GitHub (opens in a new tab)") to external links using `target="_blank"`. Always add `tabIndex={-1}` to the target container of a "skip-to-content" link so it can be focused.
