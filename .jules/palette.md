...
## 2024-11-04 - Abstract Focus-Visible Polyfills for Interactive Elements
**Learning:** In purely inline-styled components (where CSS files aren't available), interactive elements like buttons lack reliable focus states across browsers if their borders and backgrounds are reset. While adding `onFocus` and `onBlur` handlers to polyfill `:focus-visible` restores this crucial keyboard accessibility, duplicating these inline handlers across multiple buttons triggers code duplication failures in CI/CD (like SonarCloud).
**Action:** Abstracted `handleFocusVisible` and `handleBlur` handlers using `useCallback` at the component level to provide clean, unified focus-visible styling for all interactive elements within the component without code duplication.
