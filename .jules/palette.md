...
## 2024-08-07 - Clean inline style removal for focus management
**Learning:** Setting inline styles like `outline: "none"` on blur causes permanent DOM mutation, preventing elements from correctly falling back to default CSS styles if themes or contexts change.
**Action:** When cleaning up imperative inline styles in `onBlur`, set the values to empty strings (`""`) rather than `"none"` or other hardcoded reset values.
