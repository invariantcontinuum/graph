...
## 2026-06-06 - Replacing role="application" with role="region"
**Learning:** For custom interactive widgets like our WebGL graph canvas, using `role="application"` aggressively intercepts many standard screen reader reading keys, making the component hostile to users trying to explore its content sequentially.
**Action:** Replaced `role="application"` with `role="region"` alongside `aria-roledescription="graph"` and `aria-keyshortcuts="Escape + - ="` on the main `<canvas>` element. This provides a clear landmark and standard interaction model while respecting screen reader navigation.

## 2026-06-06 - Removing Redundant role="presentation"
**Learning:** SonarCloud correctly flags non-interactive overlay canvases with `role="presentation"` (`typescript:S6819`) because applying `role="presentation"` to a `<canvas>` that already has `aria-hidden={true}` and `pointerEvents: "none"` is redundant and noisy.
**Action:** Removed `role="presentation"` from all non-interactive overlay canvases in the React bridge, relying purely on `aria-hidden={true}` and `pointerEvents: "none"` to keep them out of the accessibility tree.

## 2026-06-06 - Empty State Actionability
**Learning:** Generic empty states like "No active node." provide status but fail to guide the user. Empty states should teach the next action.
**Action:** Updated the Showcase connection inspector empty state to explicitly say "Click a node to view details." to better afford interaction.
