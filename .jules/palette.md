...
## 2026-06-13 - Improve Keyboard Shortcut Discoverability
**Learning:** Screen readers and assistive technologies often struggle with parsing literal symbols like `+` or `-` in `aria-keyshortcuts` attributes. Using literal `+` can be interpreted as joining keys together instead of identifying the actual key.
**Action:** When adding `aria-keyshortcuts`, spell out symbol names using explicit words like "Plus" or "Minus" to ensure proper pronunciation and avoid ambiguity for screen readers. In addition, advertise the keyboard shortcuts consistently on active UI elements like buttons to aid discoverability.
