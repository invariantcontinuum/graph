...
## 2024-10-31 - Polyfill interaction states in inline-styled library components
**Learning:** When building React UI components for a library that strictly uses inline styles (to avoid external CSS dependencies), interactive elements like `<button>` and `<input>` inherently lack the browser's default CSS `:hover` feedback. Furthermore, applying `outline: "none"` to reset borders or relying on defaults can strip away explicit keyboard focus rings. Relying entirely on inline styles means these crucial UX and accessibility interaction states are lost by default.
**Action:** Explicitly polyfill `:hover` and `:focus-visible` states using React's event handlers (`onMouseEnter`, `onMouseLeave`, `onFocus`, `onBlur`) on all inline-styled interactive elements to preserve visual feedback for pointer and keyboard users.
...
## 2024-11-06 - Cleanly reset inline focus styles and avoid ARIA container roles on Canvas
**Learning:** For interactive `<canvas>` elements, assigning container-like roles such as `role="region"` or `aria-roledescription="graph"` triggers accessibility linter errors and can be confusing, as canvas elements are not native containers. Furthermore, when polyfilling focus styles with inline styles, removing the style by setting it to `"none"` is a permanent DOM mutation that prevents the browser's default behavior from working.
**Action:** Rely on `tabIndex={0}` and `aria-label` for keyboard accessibility on `<canvas>` without invalid ARIA container roles. When removing inline focus styles in `onBlur`, always reset the style to an empty string `""` instead of `"none"`. Additionally, use `e.currentTarget` instead of `e.target` in React synthetic events to ensure TypeScript correctly infers the DOM element type.
