...
## 2024-10-24 - Accessibility: Focusable interactive canvas and external link warnings
**Learning:** For interactive canvas surfaces like `GraphStage`, relying solely on keyboard event listeners on the `window` or `document` isn't enough for robust a11y. Screen readers and keyboard navigation require the container to be focusable programmatically (`tabIndex={-1}`) to receive keyboard input, especially when linked from a "skip-to-content" link. External links using `target="_blank"` cause a sudden context switch that is disorienting for screen reader users without an explicit warning.
**Action:** Always add `tabIndex={-1}` to target containers of "skip-to-content" links. Always pair `aria-label` with an explicit warning (e.g., "(opens in a new tab)") when using `target="_blank"` on external links.
