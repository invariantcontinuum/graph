...
## 2026-08-01 - [Short-circuit redundant Canvas2D rendering using a unified dirtyRef]
**Learning:** In canvas overlay rendering, clearing and redrawing the canvas in a continuous `requestAnimationFrame` loop when no state (camera viewport, layout, or edges) has changed causes unnecessary CPU and GPU usage on idle frames. Checking WebGL float buffers by reference (e.g. `vp === lastVp`) fails since the engine mutates them in place.
**Action:** Extracted a shared `useDprCanvas` hook to provide a `dirtyRef` flag that is correctly forced true on React renders and canvas resizes. Applied this to all canvas overlays so `requestAnimationFrame` callbacks can short-circuit early and avoid redrawing the same pixels, greatly reducing idle power consumption and increasing baseline performance.
