...
## 2026-07-16 - [Short-circuit redundant Canvas2D redraws on idle frames]
**Learning:** In React Canvas2D overlays driven by requestAnimationFrame, unconditionally redrawing every frame wastes CPU/GPU cycles when the scene is idle. We cannot check `vpMatrix !== lastVp` because WebGL engines mutate the buffer in-place. We must use a `dirtyRef` flag set in the engine callback and during React renders. Furthermore, duplicating this resize/dirty check across multiple overlays triggers SonarCloud code duplication failures.
**Action:** Extract a `useDirtyCanvas` hook to manage canvas resize detection and the dirty flag to short-circuit the render loop when idle. Ensure `dirty.current = true` is set in the component's render body (so prop updates trigger redraws) and ensure `ctx.clearRect()` happens after the early exit.
