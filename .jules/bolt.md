...
## 2026-06-12 - Fix Circular Import with WASM
**Learning:** Extracting logic that uses types generated by `wasm-pack` (like `RenderEngine`) into a separate React hook file can sometimes create circular dependency issues during the build because the package exports its own source code types.
**Action:** When extracting React hooks that need a ref to a WASM class and an import causes circular dependencies or build failures, temporarily type the ref as `React.MutableRefObject<any>` or use a structural type instead of directly importing the WASM generated class from the package index.
