...
**Learning:** In Rust hot paths like quadtree traversal (e.g., the Barnes-Hut algorithm), unpredictable spatial branching (`if x < mx`, `y < my`) causes CPU pipeline stalls.
**Action:** Replacing nested `if/else` blocks with branchless bitwise operations (e.g., `((x >= mx) as usize) | (((y >= my) as usize) << 1)`) yields measurable execution speedups for unpredictable coordinate classification.
## 2026-06-23 - [Memoize GraphTheme overrides in mergeTheme]
**Learning:** In the React bridge, `mergeGraphTheme` is called on every render to merge custom `themeOverrides`. Since users often provide inline objects for overrides (e.g. `themeOverrides={{...}}`), `mergeGraphTheme` re-evaluates the merge, creating a new theme object on every render. This creates a new identity for the theme, which propagates down to hooks like `useMemo(() => graphThemeToEngineJson(graphTheme), [graphTheme])` and invalidates caches, causing cascading identity drops and deep re-computations in downstream caches like WebGL conversions.
**Action:** When merging user-provided override objects to create configurations, employ a module-level `WeakMap` cache matching the stable base object to a `Map` keyed on the serialized overrides (e.g. `JSON.stringify(overrides)`). This ensures we return referentially stable configurations across renders when the same options are passed.
