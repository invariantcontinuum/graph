...
## 2026-07-21 - [Skip redundant Canvas2D redraws on idle frames]
**Learning:** In React Canvas2D overlays, using `requestAnimationFrame` to clear and redraw on every tick causes high CPU/GPU usage even when the graph is idle. Reference equality on WebGL engine `Float32Array` buffers (like `vpMatrix`) cannot be used to detect changes since they mutate in-place.
**Action:** Implemented a shared `useDirtyCanvas` hook that maintains a boolean `dirtyRef`. The flag is set to true during engine frame subscriptions or canvas resize events, and is cleared to false after the early exit check during the render tick, ensuring `ctx.clearRect()` only executes when a redraw is necessary. To prevent reactivity regressions, `markDirty()` is explicitly called within the render `useEffect` so that React prop updates (e.g. theme changes) correctly trigger a redraw.
