...
## 2026-06-11 - Elide Bounds Checks With Explicit Slice Length Asserts
**Learning:** In Rust layout hot paths, replacing safe array accesses directly with `unsafe { slice.get_unchecked(index) }` introduces significant memory safety risks (Undefined Behavior) if preconditions (like slice length) are not absolutely guaranteed.
**Action:** Implement "bounds check hoisting" by explicitly asserting slice lengths before the loop (e.g., `assert!(forces.len() >= n)`). This provides LLVM's Scalar Evolution (SCEV) with the necessary invariants to prove that loop indices will never exceed slice boundaries, allowing the compiler to safely and automatically elide the bounds checks inside the hot loops without needing `unsafe` blocks.
