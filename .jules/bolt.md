...
## 2026-07-17 - [Short-circuit redundant Canvas2D redraws using a dirty flag]
**Learning:** When short-circuiting a `requestAnimationFrame` render loop for Canvas2D to skip idle frames, avoiding unnecessary redraws significantly improves performance. To do this safely, we must use a boolean `dirtyRef` flag set to `true` in engine callbacks and explicitly inside the React render body (to catch prop updates), while taking care to place the `ctx.clearRect()` call *after* the early exit check. Furthermore, duplicate dirty-flag logic across components should be abstracted to prevent SonarCloud 'Duplication on New Code' failures.
**Action:** Abstract canvas dirty-flag logic into a reusable hook (`useDirtyCanvas`) that sets `dirtyRef.current = true` on every render and exposes a method to check and clear it. Apply this early-exit pattern before `ctx.clearRect()` in all continuous render loops.
