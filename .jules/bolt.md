...
## 2024-11-20 - [Memoize node type uppercasing in hot render loop]
...
**Action:** Memoize these conversions using a lazy cache (`useRef(new Map())`) evaluated dynamically during the render tick, extending existing per-item caches (e.g., `labelCache`) rather than creating new parallel `Map` refs.
