...
## 2026-07-21 - [Extract redundant requestAnimationFrame loops into renderFrame hook]
**Learning:** Manual `requestAnimationFrame` and context acquisition blocks inside React overlays are often flagged as code duplication by SonarCloud, and are prone to exhaustive-deps linting errors if functions are omitted.
**Action:** Extract repetitive Canvas2D loop logic into a centralized `renderFrame` callback inside a shared hook (`useDirtyCanvas`). To satisfy React linting, ensure the stable callback handles reference checking, dirty-state clearing, context acquisition, and early returns internally.
