...
## 2026-07-31 - [Short-circuit redundant Canvas2D redraws on idle frames]
**Learning:** In React Canvas2D overlays, `requestAnimationFrame` render loops that redraw on every frame consume unnecessary CPU/GPU resources when the graph is idle. Because WebGL engine buffer references (like `vpMatrix`) mutate in-place, simple reference equality checks fail. Using a `dirtyRef` flag set to `true` on engine updates, canvas resizes, or React prop changes, and cleared during the render tick, allows skipping redundant Canvas2D redraws on idle frames.
**Action:** Implemented a `dirtyRef` early-exit check in all Canvas2D overlays (`LabelOverlay`, `GridOverlay`, `CompoundFramesOverlay`, `EdgeLabelsOverlay`), ensuring `ctx.clearRect()` is called after the early exit to prevent blank visual regressions.
