...
**Learning:** In Rust hot paths like quadtree traversal (e.g., the Barnes-Hut algorithm), unpredictable spatial branching (`if x < mx`, `y < my`) causes CPU pipeline stalls.
**Action:** Replacing nested `if/else` blocks with branchless bitwise operations (e.g., `((x >= mx) as usize) | (((y >= my) as usize) << 1)`) yields measurable execution speedups for unpredictable coordinate classification.
## 2026-06-28 - [Memoize GraphTheme overrides merge to prevent downstream churn]
**Learning:** In the React bridge, when `GraphScene` merges app-provided `themeOverrides` with the base theme via `mergeGraphTheme`, inline object literal props for `themeOverrides` cause referential identity changes on every render. Because the merged theme cascades into `graphThemeToEngineJson` (which uses `WeakMap` based on theme object identity to prevent deep JSON conversion), un-memoized merges bust the JSON cache, causing unnecessary JSON serialization and garbage collection.
**Action:** When creating merged configuration objects from inputs that are frequently inline object literals, employ a two-level cache (e.g., `WeakMap` for the stable base, inner `Map` bounded by size for the serialized dynamic inputs) to return referentially stable objects and protect downstream caches.
