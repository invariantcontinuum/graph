...
**Learning:** When skipping redundant Canvas2D redraws on idle frame updates, do not use reference equality checks on WebGL engine Float32Array buffers, as the engine mutates these in-place. Instead, use a boolean dirtyRef flag that is set to true in the engine's subscription callback and cleared to false during the render tick, explicitly accounting for canvas resize events and placing clearRect after the early exit check. To avoid CI duplication failures when repeating identical logic across multiple canvas overlays, extract this pattern into a shared hook (useDirtyCanvasFrame).
**Action:** Added dirtyRef short-circuit logic to GridOverlay, EdgeLabelsOverlay, CompoundFramesOverlay, and LabelOverlay via a shared useDirtyCanvasFrame hook to pause work when the frame data hasn't changed.
