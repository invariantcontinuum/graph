...
**Learning:** In React Canvas2D overlays driven by requestAnimationFrame, unconditionally redrawing every frame wastes CPU/GPU cycles when the scene is idle. We cannot check `vpMatrix !== lastVp` because WebGL engines mutate the buffer in-place. We must use a `dirtyRef` flag set in the engine callback and during React renders.
**Action:** Add a `dirtyRef` flag to short-circuit the render loop when idle. Ensure `dirtyRef.current = true` is set in the component's render body (so prop updates trigger redraws) and ensure `ctx.clearRect()` happens after the early exit.
