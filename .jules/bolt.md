...
## 2026-06-12 - Pointer Controller Extraction & Workspace Lockfile Ignore
**Learning:** Extracting complex inline pointer event logic from `react/Graph.tsx` into a dedicated custom hook `usePointerController` along with its pure helper functions (`toLocalPointer`, `handleHoverOnly`, `handleSinglePointerMove`, `handlePinchMove`, `flushWorkerMessages`) effectively resolves SonarCloud `typescript:S3776` cognitive complexity findings without altering the underlying interaction behaviors. Additionally, refining `sonar.issue.ignore.multicriteria` for `text:S8570` from `crates/**/Cargo.toml` to `crates/*/Cargo.toml` in `sonar-project.properties` provides a more precise rule suppression for nested workspace lockfile analyzer noise.
**Action:** When refactoring event orchestration components, move independent branch logic into named, testable pure functions outside the React component or hook scope. Use strict glob patterns in Sonar configuration to suppress known false positives explicitly.
