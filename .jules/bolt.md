...
## 2026-06-09 - Fix SonarCloud S3776 in Graph.tsx and Cargo.toml lockfile noise
**Learning:** React component files, particularly those with complex orchestration like `Graph.tsx`, can suffer from bloated `useEffect` blocks that cause cognitive complexity warnings. Extracting event handling (e.g., pointer gestures) into named custom hooks organizes the logic safely without requiring breaking changes to the component's public interface. For false positive lockfile warnings in SonarCloud on nested Rust workspace crates, the `sonar.issue.ignore.multicriteria.e1.resourceKey` property should use the exact path pattern `crates/*/Cargo.toml` instead of `crates/**/Cargo.toml` to successfully match and ignore the findings.
**Action:** Created `usePointerEvents` hook in `react/usePointerEvents.ts` to encapsulate all complex pointer state from `Graph.tsx`. Updated `sonar-project.properties` to properly ignore nested `Cargo.toml` missing lockfile warnings (`text:S8570`).
