...
## 2026-07-12 - [Skip redundant Canvas2D redraws on idle frames]
**Learning:** In interactive graph visualizations, Canvas2D overlays (like labels, grids, and frames) redraw constantly on idle frames if they only rely on a raw `requestAnimationFrame` loop without checking if the underlying layout data actually changed. Using reference equality checks on WebGL engine `Float32Array` buffers fails because the engine mutates these in-place.
**Action:** Use a boolean `dirtyRef` flag that is set to `true` in the engine's subscription callback and cleared to `false` during the render tick. Explicitly check that both `!dirtyRef.current` and canvas dimensions match before skipping `clearRect` and redraws in the tick loop to drastically save CPU/GPU cycles on idle scenes.
