...
## 2026-07-15 - [Split worldToScreen to bypass object allocation in render loops]
**Learning:** In hot render paths (like `requestAnimationFrame` loops for canvas overlays), utility functions returning short-lived parameter objects (like `{sx, sy}`) cause high memory churn and GC pauses when called for thousands of nodes.
**Action:** Split coordinate projection functions (like `worldToScreen`) into distinct primitive-returning functions (e.g., `worldToScreenX` and `worldToScreenY`) to completely bypass object allocations.
