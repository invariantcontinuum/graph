...
## 2026-07-18 - [Split projection functions to avoid object allocation in hot paths]
**Learning:** Returning short-lived objects (like `{sx, sy}`) from highly-called math functions (like `worldToScreen`) inside per-frame canvas rendering loops causes significant memory churn and GC pauses.
**Action:** Split coordinate projection functions into separate `X` and `Y` primitive functions (e.g., `worldToScreenX` and `worldToScreenY`) to avoid short-lived object allocation completely.
