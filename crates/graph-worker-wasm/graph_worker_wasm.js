/* @ts-self-types="./graph_worker_wasm.d.ts" */

/**
 * Called from JS worker bootstrap on each `onmessage`.
 * @param {any} msg_js
 */
export function handle_message(msg_js) {
    const ret = wasm.handle_message(msg_js);
    if (ret[1]) {
...
function __wbg_get_imports() {
    ...
    return {
        __proto__: null,
        "./graph_worker_wasm_bg.js": import0,
    };
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('graph_worker_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
